...
const productCategoryPage = async (req, res) => {
    try {
        ...
        res.cRender('product/product-category.pug', {result: result[1], attributes:result[1], products: mapProductData.rows, count: mapProductData.count, title: result.name, pagination: true });
    } catch (error) {
        logger.error(error);
    }
}
